...
function Table({ filterProps, filterData, seoName, fields }) {
  ...
  useEffect(() => {
    fetchData();
  }, [pageIndex, pageSize, filterProps]);
                  ...
                  <React.Fragment key={i}>
                    <tr {...row.getRowProps()}>
                      {row.cells.map((cell, j) => {
                        console.log("ddddd", cell.row.original.Image)
                        if (cell.column.id === 'image') {
                          ...
                          return (
                            <td key={j} {...cell.getCellProps()} className='link-success  text-decoration-underline borderMavi'>
                              <Link className='borderMavi' href={`/product/${encodeURIComponent(cell.row.original.PartNumber)}-${cell.row.original.urun_id}`}>{cell.row.original.PartNumber}</Link>
                            </td>)
                        }
